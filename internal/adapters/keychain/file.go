...
func LiveSessionPath() string {
	if d := os.Getenv("XDG_STATE_HOME"); d != "" {
		return filepath.Join(d, "atlas", "session.json")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".local", "state", "atlas", "session.json")
}

func FakeSessionPath() string {
	if d := os.Getenv("XDG_STATE_HOME"); d != "" {
		return filepath.Join(d, "atlas", "fake-session.json")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".local", "state", "atlas", "fake-session.json")
}
